/*
  Last changed Time-stamp: <2005-02-18 14:12:21 ivo>
  c  Christoph Flamm and Ivo L Hofacker
  {xtof,ivo}@tbi.univie.ac.at
  Kinfold: $Name:  $
  $Id: baum.c,v 1.3 2005/02/18 13:29:04 ivo Exp $
*/
...
static char UNUSED rcsid[]="$Id: baum.c,v 1.3 2005/02/18 13:29:04 ivo Exp $";
...
static void ini_ringlist(void) {
  ...
  for(i = 0; i < GSV.len; i++) {
    ...
    pos = strchr(Law_and_Order, GAV.farbe[i]);
    if ( pos == NULL ) {
      rl[i].base = 0;
      typeList[i+1] = 0;
    }
      ...
      typeList[i+1]= pos - Law_and_Order;
    }
...
static int bp_game (baum *root) {
  ...
  for (rli = stop->next; rli != stop; rli = rli->next) {
    for (rlj = rli->next; rlj != stop; rlj = rlj->next) {
      ...
      if (pair[rli->base][rlj->base]) chords++;
    }
  }
  return(chords);
}
